...
static RPCHelpMan getpolmineridstatus()
{
    return RPCHelpMan{
        "getpolmineridstatus",
        "Return PoL miner-tag tracking status (patchless tagging via extranonce2 prefix).\n"
        "\nThis RPC is tracking-only in Beta7: it does not change consensus or cap rewards.\n",
        {
            {"miner_tag_hex", RPCArg::Type::STR, RPCArg::Optional::NO, "Miner tag in hex (8/16/24 hex chars for 4/8/12 bytes)."},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::NUM, "tip_height", "Current chain tip height."},
                {RPCResult::Type::STR, "miner_tag_hex", "Miner tag as provided (normalized hex)."},
                {RPCResult::Type::NUM, "miner_tag_len", "Tag length in bytes."},
                {RPCResult::Type::NUM, "miner_tag_u32", "First 4 bytes interpreted as little-endian uint32 (for legacy 4-byte tags)."},
                {RPCResult::Type::NUM, "extranonce1_size", "Configured extranonce1 size used for tag extraction (-pol_extranonce1size)."},
                {RPCResult::Type::BOOL, "seen", "Whether this tag has been observed on-chain by this node."},
                {RPCResult::Type::NUM, "first_seen_height", "First block height where this tag was observed (or -1)."},
                {RPCResult::Type::NUM, "last_seen_height", "Most recent block height where this tag was observed (or -1)."},
                {RPCResult::Type::NUM, "blocks_seen", "How many connected blocks had this tag (as block finder)."},
                {RPCResult::Type::NUM, "last_seen_time", "Unix time of the last seen block (or 0)."},
            }
        },
        RPCExamples{
            HelpExampleCli("getpolmineridstatus", "0408ee46")
            + HelpExampleRpc("getpolmineridstatus", "0408ee46")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            const NodeContext& node = EnsureAnyNodeContext(request.context);
            ...
            const std::string hex_in = request.params[0].get_str();
            const std::vector<unsigned char> tag = ParseHex(hex_in);

            if (!(tag.size() == 4 || tag.size() == 8 || tag.size() == 12)) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "miner_tag_hex must be 4, 8 or 12 bytes (8/16/24 hex chars)");
            }

            UniValue obj(UniValue::VOBJ);
            obj.pushKV("tip_height", chainman.ActiveChain().Tip() ? chainman.ActiveChain().Tip()->nHeight : -1);
            obj.pushKV("miner_tag_hex", HexStr(tag));
            obj.pushKV("miner_tag_len", (int)tag.size());

            // miner_tag_u32 (legacy convenience)
            uint32_t u32 = 0;
            if (tag.size() >= 4) {
                u32 = (uint32_t)tag[0] | ((uint32_t)tag[1] << 8) | ((uint32_t)tag[2] << 16) | ((uint32_t)tag[3] << 24);
            }
            obj.pushKV("miner_tag_u32", (uint64_t)u32);

            obj.pushKV("extranonce1_size", pol::GetConfiguredExtraNonce1Size());

            const auto st = pol::GetMinerTagStatus(tag);
            if (!st.has_value()) {
                obj.pushKV("seen", false);
                obj.pushKV("first_seen_height", -1);
                obj.pushKV("last_seen_height", -1);
                obj.pushKV("blocks_seen", 0);
                obj.pushKV("last_seen_time", 0);
                return obj;
            }
            ...
            obj.pushKV("blocks_seen", (uint64_t)st->blocks_seen);
...
void RegisterBlockchainRPCCommands(CRPCTable& t)
{
    static const CRPCCommand commands[]{
        ...
        {"blockchain", &getpolmineridstatus},
